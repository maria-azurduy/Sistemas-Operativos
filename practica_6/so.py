...
class Loader():
    ...
    def getProgramFromDisk(self,path):
        ## Lee el programa del FS y maneja el error.
        program = self._fileSystem.read(path)
        if program is None:
            log.logger.error(f"LOADER: Error, programa no encontrado en '{path}'")

        progSize = len(program.instructions)
        return program, progSize
    
    def loadProgram(self, path):
        ##crea la pageTable
        program,progSize = self.getProgramFromDisk(path)
        ...
        ## PageTable: Mapea { 0: {'valid': False, 'frame': None}, 1: {'valid': False, 'frame': None}
        ## Valid False significa que no está en RAM aun.
        ## FRAME None significa que aun no se le asignó un frame (xq no está en RAM).   
        pageTable = {} ## es diccionario 
        for pageIndex in range(numPagesNeeded): 
            pageTable[pageIndex] =  {'valid':False, 'frame':None} ## cada página del programa cargado es mapeada con VALID: FALSE Y FRAME:NONE
        
        log.logger.info(f"LOADER: Creada la Page Table para '{path}'" 
                        f"con {numPagesNeeded} páginas")
        return pageTable 
    
    def loadPage(self, path, pageNum, frameNum):
        ##Carga UNA página específica de un programa (desde el disco)
        ##en UN frame específico (en la RAM).
        
       program,progSize = self.getProgramFromDisk(path)
        #Calcula qué pedazo del programa cargar
        #    (Las instrucciones de esta página)        
        # Dirección lógica de inicio de esta página
       startAddr = pageNum * self._pageSize ## ("crece para arriba")
        
       # Dirección lógica de fin de esta página
       # (No puede pasarse del tamaño total del programa)
       endAddr = min(startAddr + self._pageSize, progSize)
       log.logger.info(f"LOADER: Cargando página {pageNum} (instrs {startAddr} a {endAddr}) "
                       f"del programa '{path}' en el frame {frameNum}")
       
       # Por cada dirección del programa 'logicalAddress' itera y obtiene:
       for logicalAddress in range(startAddr, endAddr): ## logicalAdress == índice 
           
           # la instrucción del programa
           instruction = program.instructions[logicalAddress]
           
           # La ubicación de esta instrucción dentro del frame
           offset = logicalAddress % self._pageSize
           
           # La dirección física final en la RAM
           physicalAddr = (frameNum * self._pageSize) + offset
           
           # Escribir en la memoria física
           HARDWARE.memory.write(physicalAddr, instruction)

class Dispatcher():
   ## { 0: {'valid': False, 'frame': None}, 1: {'valid': False, 'frame': None}}
    def load(self,pcb):
        HARDWARE.mmu.resetTLB()

        pageTable = pcb.pageTable 
        for pageNum, frameEntry in pageTable.items():
            if frameEntry['valid']:
                frameNum = frameEntry['frame']
                HARDWARE.mmu.setPageFrame(pageNum, frameNum)
            else :
                HARDWARE.mmu.setPageFrame(pageNum, None)
...
class KillInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        pageTable = pcb.pageTable
        pcbFrames = [] ##extrae los frames del diccionario (pagetable) como una lista.
        for e in pageTable.values():
            ## solo si tiene un frame asignado (NO ES NONE)
            if e ['frame'] is not None:
                pcbFrames.append(e['frame'])
        self.kernel.memoryManager.releaseFrames(pcbFrames)
        self.kernel.cleanFramesQueue(pcbFrames)
...
class PageFaultInterruptionHandler(AbstractInterruptionHandler):
     def execute(self, irq):
        ## El proceso intentó acceder a una página que no está en memoria.

        pcb = self.kernel.pcbTable.runningPCB
        pageNum = irq.parameters
        log.logger.error(f"HANDLER (PAGE FAULT): PID {pcb.pid} falló al acceder a la página {pageNum}")
        
        frameIndex = self.kernel.memoryManager.allocateFrames(1) ##pide a la MM un frame libre
        
        if frameIndex is None:
                ##no hay memoria suficiente
                log.logger.error(f"HANDLER (PAGE FAULT): No hay memoria. Empleando mecanismo de reemplazo...")
                frameNum = self.kernel.handlePageReplacement() ## devuelve el nuevo frame libre
        else:
            frameNum = frameIndex[0] ## Saca el único elemento de la lista de frames (el que se solicitó)
            pck = (frameNum, pcb, pageNum)
            self.kernel._framesQueue.append(pck) ##se va llenando la frameQueue con cada frame, pagina y el pcb asociado. 
        
        self.kernel.loader.loadPage(pcb.path, pageNum, frameNum) ##Loader lee las instrucciones correspondientes a la página pageNum desde el fileSystem y las escribe en la RAM,en el Marco frameNum
        pcb.pageTable[pageNum]['valid'] = True     
        pcb.pageTable[pageNum]['frame'] = frameNum     
        #Actualiza la TLB                                ## ej: 1: {'valid': True,  'frame': 5}
        HARDWARE.mmu.setPageFrame(pageNum, frameNum)
...
class Kernel():
    ...
    def __init__(self,memorySize, pageSize):
       ...
       ##Las estadisticas se habilitan seteando el flag enable_stats del CPU
        ...
        # La cola FIFO ahora guardará tuplas:
        # (frameNum, pcb, pageNum)
        self._framesQueue = []  ## para implementar FIFO en el algoritmo de selección de victima. 
    ...
    def handlePageReplacement(self):
        victimFrame,victimPCB,victimPage = self.chooseVictimAlgorithm()
        self.evictVictim(victimFrame,victimPCB,victimPage)

        return victimFrame
    
    def chooseVictimAlgorithm(self):
        ## decide que frame sacar
        victimTuple = self._framesQueue.pop(0)
        log.logger.info(f"REEMPLAZO FIFO: Víctima seleccionada: {victimTuple}")
        return victimTuple  # Devuelve (frame, pcb, page)

    def evictVictim(self,victimFrame,victimPCB,victimPage):
            victimPCB.pageTable[victimPage]['valid'] = False
            victimPCB.pageTable[victimPage]['frame'] = None

            #Limpia la caché de la TLB
            HARDWARE.mmu.resetTLB()
            log.logger.info(f"REEMPLAZO FIFO: Evictado frame {victimFrame}")

    def cleanFramesQueue(self,freeFrames):
            log.logger.info(f"Limpiando cola FIFO de frames {freeFrames}")
    
     ## Esto crea una nueva cola solo con las entradas cuyo frame (entry[0]) no esté en la lista de liberados.
            self._framesQueue = [entry for entry in self._framesQueue
                        if entry[0] not in freeFrames]
